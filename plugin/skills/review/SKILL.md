...
Walk `deliverables/` and `specs/blocked/`, oldest first, blocked items before
deliverables (they gate other work). One item at a time; apply the verdict
fully before showing the next.
...
entries in `contracts/PROPOSALS.md` (rule changes the per-item retros queued
— offer to walk them at the end of the pass), plus any **stale-PR flags** —
...
1. Present: title, what was built (from `DELIVERABLE.md`), the acceptance
   ...
   **adversarial review verdict** from `REVIEW.md` (per
   `contracts/REVIEWER.md`). Deliverables only exist post-`ship` — the PR
...
3. Take the verdict:
   - **Approve** → for `pr` deliverables: `gh pr ready <url>` (draft → ready
     for the team's normal review), then move `deliverables/<id>/` →
     `archive/<id>/`, commit `approve: <id>`. For reports: publish per the
     spec's stated destination, then archive.
   - **Reject** → require the routing, never accept a bare no:
     - *fast* (spec was wrong/incomplete): capture the reason in
       ...
       to `specs/inbox/<id>/`, commit `reject-fast: <id>`.
...
- Merged PRs found during the stale-check → move their items to `archive/`
  automatically and mention it (merge was the approval's completion).
